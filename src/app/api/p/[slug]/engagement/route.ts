...
export async function POST(
  ...
  const existing = couple.engagement_photos ?? [];
  if (existing.length >= MAX_PHOTOS) {
    return NextResponse.json(
      { error: `Maksimum ${MAX_PHOTOS} fotoğraf eklenebilir.` },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    await getAdminDb()
      .collection("couples")
      .doc(couple.id)
      .update({ engagement_photos: FieldValue.arrayUnion(url) });
    revalidatePath(`/${slug}`);
...
export async function DELETE(
  ...
  if (!url) {
    return NextResponse.json(
      { error: "Hangi fotoğrafın silineceğini belirtin." },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    await getAdminDb()
      .collection("couples")
      .doc(couple.id)
      .update({ engagement_photos: FieldValue.arrayRemove(url) });
    revalidatePath(`/${slug}`);
    revalidatePath(`/p/${slug}/duzenle`);
